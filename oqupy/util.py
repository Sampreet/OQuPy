...
def create_deltas(
    ...
    for i in indices:
        scrambled_tensor = np.zeros(tuple(_shape[_idxs]), \
                                    dtype=tensor.dtype)
        scrambled_tensor[indices_out] = func_tensors(i)[indices_in] / scale
        scrambled_tensors.append(scrambled_tensor)
...
def create_delta(
    ...
    scrambled_tensor = np.zeros(tuple(_shape[_idxs]), \
                                dtype=tensor.dtype)
    scrambled_tensor[indices_out] = tensor[indices_in] / scale
